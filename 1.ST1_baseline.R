...
library(data.table)
cov=fread('COV_ZJ_JS_imaging_39334.txt') #所有协变量都在先前gwas分析用到的cov文件中，直接局域此文件进行分析
cov=cov[,-c(1:2)]
...
table(is.na(cov1$sex)) #35469人，正确
...
write.table(tab4Mat3,'baseline_男女.xlsx',quote = F,sep = '\t',row.names = T)
